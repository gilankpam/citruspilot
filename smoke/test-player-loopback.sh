#!/bin/sh
# test-player-loopback.sh — integration test for the citrusplay HW-decode path.
# RUNS ON THE GS. Loops a clean, zero-loss in-band H.265 file in as RTP and
# asserts citrusplay actually HW-decodes and PRESENTS frames to the plane.
#
# This is the regression test for the RTP -> stateless-Cedrus stall: without the
# in-decoder HEVC parser, citrusplay gets 0 DRM_PRIME frames out of a *clean* stream
...
BIN="${CITRUSPLAY:-/usr/local/bin/citrusplay}"
PORT="${PORT:-5600}"
PT="${PT:-97}"
DUR="${DUR:-14}"
MIN_FRAMES="${MIN_FRAMES:-30}"
LOG=/tmp/test-citrusplay.log
...
# citrusplay itself takes DRM master and force-sets its own socket buffer)
...
echo "FAIL: citrusplay did not present a healthy frame stream"
echo "--- last log lines ---"
